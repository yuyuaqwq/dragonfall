...
# 框架的 Database.lock 即为 RLock（同一条理由写进了它的 docstring）。
_lock = _db.lock


def _connect():
    """开一条新连接（其余 store 模块的既有调用点保持不变）。"""
    return _db.connect()


def atomic():
    """原子写上下文（F1 P0-2 单连接 + 单事务 + BEGIN IMMEDIATE）—— 见框架实现。"""
    return _db.atomic()
...
_db.register_schema("core", _SQL_CORE_TABLES)
_db.register_schema("social", _SQL_SOCIAL_TABLES)
_db.register_schema("professions", _SQL_PROF_TABLES)
_db.register_schema("identity", _SQL_IDENTITY_TABLES)


def init_db():
    """建表 + 老库补列自愈。

    实现见框架 `Database.init()`：注册序 executescript → 迁移器 → 一次提交（幂等）。
    """
    _db.init()
...
_db.register_migration(_ensure_legacy_columns)
