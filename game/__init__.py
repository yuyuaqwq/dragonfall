...
引擎装配器 `game/bootstrap.py` 并调用 `install()`：
  ...
  * 登记 saintess_engine 引擎的「默认配置装载器」（旧 `load_game_defaults()` 委托目标，R7）
...
方向：内容 → 引擎（合法）。引擎（`framework/saintess_engine/`）自身零内容 import，
...
# `framework/`（内含包 `saintess_engine/`）。必须在任何 `import saintess_engine` 之前把框架根
