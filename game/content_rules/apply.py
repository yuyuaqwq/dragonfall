...
--------------------------------------------------------------------------
    ① ensure_engine_configured()          引擎 hook + 规则表装配（幂等；先于一切内容装配）
                                          —— 等价旧 `game.battle2.config.load_game_defaults()`
...
方向性：本模块在**内容侧**（content_rules）——引擎 `game/battle2/` 不得 import 本模块
...
def ensure_engine_configured() -> None:
    """引擎配置**一次性**装配（幂等）——旧 `game.battle2.config.load_game_defaults()` 的实体。
