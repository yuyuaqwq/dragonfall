# -*- coding: utf-8 -*-
"""团队/全队类技能效果装配层 battle_team_procs（2026-09-11）。

## 为什么需要这一层

引擎的增益管线（`saintess_engine.battle.actions._do_buff`）把 `effect=` 名词翻译成动作后
**只作用于施法者自己**（`apply_effects(battle, actor, actor, ...)`）。所以历史上
`atk_all` / `reduce_all` / `shield_all` 这类「全队」技能实际只给自己加——单人战斗时代
「全队 = 自己」无感，多人副本/PVP 下就是**描述承诺与行为不符**。

本模块提供**团队面幅**（遍历同侧存活 actor）+ 需要它的语义族，**零引擎改动**。

## 口径（鱼鱼 2026-09-11 拍板）

- **减伤 = 叠加（乘算）**：多个减伤来源各自 `mult *= (1-r)` 进 `taken_calc`，
  即战士 30% + 牧师 20% → ×0.7×0.8 = 44%。
- **护盾 = 百分比与固定值都支持，按技能设定**：`shield_pct`（施法者生命上限百分比）/
  `shield_per_stack` + `shield_res_key`（按资源层数递增）/ `shield_value`（固定数值）。
  缺字段 = 无此行为（零默认值铁律）。
- **刻数过期**统一走 `effects[key] = {..., "expire": now + turns}`（引擎
  `_settle_time_effects` 自动清理），触发器只做「态在 → 改乘区」——即既有
  `class_stance_guard_enter`「写态 + 挂 trigger」模式的推广，不新造计时器。
- **数值全部读技能数据**（`params["info"]`，由 `_do_buff` 注入）——引擎与本模块都不写死。

## 动作清单

| 动作 | 面幅 | 用途 |
|---|---|---|
| `team_apply` | 同侧全队 | 通用：把引擎既有 `apply` 逐个发给全队（面板类 `*_all`） |
| `team_shield` | 同侧全队 | 全队护盾（pct / per_stack / value 三形态） |
| `self_shield` | 自身 | 自身护盾（同口径，装备/药水/自身技能用） |
| `team_taken_reduce` | 同侧全队 | 全队减伤（写态 + 挂 taken_calc 触发器，乘算叠加） |
| `team_ss_reduce_apply` | — | 触发器：读态 → `ctx.mult *= (1-reduce)` |
| `timed_vuln` | 单体 target | 目标易伤（写态 + 挂 taken_calc 到目标，带刻数） |
| `timed_vuln_apply` | — | 触发器：读态 → `ctx.mult *= (1+amp)` |
| `team_dmg_aura` | 同侧全队 | 全队伤害乘区（按伤害类型 / 目标标记过滤，挂 dmg_calc） |
| `team_dmg_aura_apply` | — | 触发器：条件判定 → `ctx.mult *= (1+add)` |
| `target_lock_mark` | 单体 target | 锁定标记（供 `team_dmg_aura.target_lock` 判定） |
| `team_cc_immune` | 同侧全队 | 全队免疫控制（写 `cc_immune` 态；引擎控制落地处消费） |
| `self_cc_immune` | 自身 | 自身免疫控制（潜行类技能的免疫段） |
"""
