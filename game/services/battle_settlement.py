# -*- coding: utf-8 -*-
"""v181 P4-9 战斗结算服务层（game/services/battle_settlement.py）

v181.P4-9：combat._handle_victory/_handle_defeat 的纯同步结算编排原样随迁
（方案 A：1790–2201 段收 service 主段；公会任务/升级/quest/野王/塔卫/成就/
rule/_next_step/收尾段 2202–2297 留命令层壳，yield 流留命令层——v3.4 铁律）。

函数名去 `_` 前缀、签名去 self，body 逐行 copy（含注释/空行/文案 emoji），
等价替换白名单（diff 只允许以下 6 类）：
  ① 去掉 self 参数
  ② self._player → db.get_player
  ③ self._title_bonus → core.title_bonus.title_bonus（同实现单点）
  ④ self._rule_fire → core.rule_engine.fire(hooks={"title_bonus": ...})
     （hooks 语义与 base._rule_fire 一致：hook 收 q → 查该 q 的 title_bonus）
  ⑤ self._nearest_town/_is_redname/_red_until → 模块内同名单点纯函数
  ⑥ 文件名/函数名变更
任何数值/概率/文案/emoji/random 调用顺序差异 = 失败回退。

依赖红线：可 import game.data / game.core（含 C 聚合）/ game.store（db）/
game.engine / game.reward / game.battle / 同级 services；禁止 import commands/*。
db 等一律函数体内惰性 import（防 data/_assembly 加载期循环，core 样板铁律）。

搬入内容：
  - victory 主段小函数（按 combat 原段顺序）：exp_curve / party_exp_bonus /
    guild_exp_bonus / pet_exp_gain / mount_exp_bonus / world_event_bonus /
    fortune_bonus / bump_kill_stats / roll_blueprint_drop / roll_equip_drop /
    roll_pet_egg / roll_mount_drop / roll_rune_drop / roll_gem_drop /
    rune_income / lucky_charm / material_fold / know_exp_bonus /
    grant_player_exp
  - 纯渲染/判定单点原样搬：next_step_hint（combat._next_step_hint 整函数）/
    nearest_town（combat._nearest_town 整函数 BFS）/
    red_until / is_redname（combat._red_until/_is_redname 整函数）/
    grant_worldboss_drop（combat._grant_worldboss_drop 整函数）
  - 大编排：victory_settle（1803–2201 段原顺序串联，返回结构化 dict）/
    defeat_settle（defeat 全同步编排，返回结构化 dict）

⚠️ 内部传递链（与 combat 原实现逐字一致）：
...
def is_redname(qq_id) -> bool:
    return time.time() < red_until(qq_id)
...
def victory_settle(group_id, qq_id, player, monster, result, extra_kills=None):
    ...
    # ---- 段2 组队 ----
    exp, party_bonus_line = party_exp_bonus(group_id, qq_id, exp)
    # ---- 段3 公会 ----
    exp, guild_bonus = guild_exp_bonus(qq_id, exp)
    # ---- 段4 宠物 ----
    exp, pet_bonus = pet_exp_gain(qq_id, exp, monster)
    # ---- 段5 坐骑 ----
    exp, mount_bonus = mount_exp_bonus(player, exp)
    # ---- 段6 世界事件 ----
    exp, gold, evt_bonus, evt_effects = world_event_bonus(group_id, qq_id, exp, gold)
    # ---- 段7 每日运势 ----
    exp, gold, fortune_line = fortune_bonus(group_id, qq_id, exp, gold)
    if fortune_line:
        evt_bonus.append(fortune_line)
    # ---- 段8 任务统计/图鉴/声望 ----
    rep_lines = bump_kill_stats(group_id, qq_id, monster, evt_effects)
    # ---- 段9 图纸掉落 ----
    drop_equip, drop_lines, gold = roll_blueprint_drop(group_id, qq_id, player, monster, gold)
    # ---- 段10 装备掉落 ----
    drop_lines = roll_equip_drop(group_id, qq_id, monster, drop_equip, drop_lines)
    # ---- 段11 宠物蛋 ----
    pet_egg_line = roll_pet_egg(group_id, qq_id, monster)
    # ---- 段12 坐骑缰绳 ----
    mount_line = roll_mount_drop(group_id, qq_id, monster)
    # ---- 段13 符文 ----
    rune_line = roll_rune_drop(group_id, qq_id, monster)
    # ---- 段14 原石 ----
    gem_line = roll_gem_drop(group_id, qq_id, monster)
    # ---- 段15 符文收益 ----
...
def defeat_settle(group_id, qq_id, player, monster, result):
    ...
    _town_id = nearest_town(player.get("cur_map", ""))
