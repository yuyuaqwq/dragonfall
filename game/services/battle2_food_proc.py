...
- 周期恢复类（regen/meditate/dawn_crown）→ actor["effects"][key] period 声明
  （schedule 周期段时间驱动每刻跳，对齐 battle2_item_use hot: regen_hot 先例；
...
缺字段 = 无此行为，不复制硬编码）。安装入口 battle2_item_use.translate foodfx
分支（吃料理唯一入口，_instance_router + _restore_battle2 全覆盖）。
"""
...
# ============================================================
# 挂载入口（battle2_item_use.translate foodfx 分支调用）
# ============================================================
...
def install_food_fx(actor: dict, aids: list, logs: list) -> None:
    ...
    if not actor or not aids:
        return
    try:
        from .battle2_equip_proc import install_ext_actions
