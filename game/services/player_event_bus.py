...
- EVENTS 起步全集固定元组；register 未知事件 raise ValueError（防拼写静默失效）；
  fire 未知事件 log warning 后返回 []（不 raise——语义扩展期容忍）。
...
# 起步全集（扩展靠 data/声明不靠加 if——但事件本体先枚举，防止拼写漂移）
EVENTS = ("battle_victory", "battle_defeat", "monster_killed")

# event -> [(subscriber, blank_line)]，注册顺序即执行/回填行顺序
_registry = {ev: [] for ev in EVENTS}
...
def register(event: str, subscriber, blank_line: bool = True) -> None:
    ...
    if event not in EVENTS:
        raise ValueError(f"[player_event_bus] 未知事件 {event!r}，EVENTS={EVENTS}")
    _registry[event].append((subscriber, bool(blank_line)))
...
def fire(event: str, ctx: dict) -> list:
    ...
    """
    if event not in EVENTS:
        _log.warning("[player_event_bus] fire 未知事件 %r，忽略", event)
        return []
    lines = ctx.setdefault("lines", [])
    ctx["event"] = event
    subs = _registry[event]
    if not subs:
        return lines
    for subscriber, blank in subs:
        try:
            ...
            _log.warning("[player_event_bus] 订阅方 %r 异常，跳过（事件 %s）",
                         getattr(subscriber, "__name__", subscriber), event, exc_info=True)
            continue
        if not seg:
            continue
