# -*- coding: utf-8 -*-
"""奥兰迪亚·余烬纪年 服务层 - profession.py（P4-7 ProfessionService 等待型副业域）

v181.P4-7：economy.py 等待型副业区（垂钓/采集/挖掘）纯逻辑服务化（函数体原样随迁
services 样板：逐行 copy 零业务行变化）——命令层只留解析 + 守卫 + async yield 壳，
业务走本模块（services 禁 import commands/*）。

搬入内容（economy.py L53/L620-1423 原样迁入；函数名去 `_` 前缀、签名去 self）：
  - timed_events 引擎注册：economy.py:53 `_te.register_timed("prof_wait", ..., on_expire=…)`
    迁到本模块顶层（economy 命令层的注册调用删除）——prof_wait_expire_cb 数据保全回调随迁。
  - 常量：MINING_FATIGUE_THRESHOLD / MINING_FATIGUE_RECOVER（v105 M14 挖掘疲劳，原 economy 模块级）、
    _GATHER_COND_CHECKERS 采集限定条件词注册表 + v125.2 启动校验（GATHER_COND_POOLS 全量 ∈ 注册表，
    fail-fast）、FISHING_SURPRISE_* 垂钓惊喜概率表/档位边界（v168.2 鱼鱼拍板，原 economy 类属性）
  - 等待型副业状态机（v55/v127.5 懒计时引擎）：prof_wait_key/ev_name/compat/residual/state/clear/
    duration/begin/prof_settle/prof_wait_flow + prof_delayed_push（async 推送壳）
  - 结算：settle_fishing（v168.2 垂钓惊喜层/鱼王/宝物/垃圾分支）、fishing_surprise（惊喜盲盒）、
    collect_bonus_line（彩蛋收藏鱼）、settle_gather（含稀有彩蛋/坐骑 collect_bonus/每日采集任务）、
    mining_fatigue_state/tick/fatigued（v105 挖掘疲劳）、settle_mining（深矿池/稀有矿脉/疲劳减半）
  - 采集掷池：gather_roll（v174 drop_engine 数据源 + 价格带兜底 + 限定采集物低权追加）、
    gather_cond_roll（v102.3 限定采集物判定 + v125.2 fail-closed）

广播/命令层能力经 hooks 注入（§2.4 hooks={"legend": callable} 缺省无副作用）：
  - 鱼王出水/古代鱼骨传说档全服广播：原 self._broadcast（依赖命令层实例 + create_task）→
    settle_fishing 收 hooks={"legend": callable}（收 group_id/qq_id/player/fname/spot）；
    缺省 None = 无广播（与原 _fish_legend_broadcast 测试环境 try/except 静默等价）。
  - v97.5 行为彩蛋（rule_engine.fire "gather_done" 触发，原 self._rule_fire）→ prof_settle 收
    rule_fire 注入（收 trigger/group/qq/player/cur_map/evt → 文本；缺省 None 不触发——命令层壳必传）。
  - 每日副业任务推进（self._daily_prof_bump）→ settle_fishing/settle_gather/settle_mining 收
    daily_prof_bump 注入（收 group/qq/tkey → (done,msg)；命令层壳必传）。
  - 每日采集任务 collect_any 推进（self._bump_daily_progress）→ settle_gather 收 collect_any_bump 注入。
  - tips 随机提示（self._tip("gather")）→ settle_gather 收 tip 注入；纯命令层渲染行。
