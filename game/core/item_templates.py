# -*- coding: utf-8 -*-
"""game/core/item_templates.py —— B13-L1 **薄壳**（2026-09-14）

真源（**唯一实现**）已搬进内容包：`framework/games/orlandia/content/item_templates.py`
（搬的边界 / 正文改动面 / 缺口全写在那边的头注里）。本文件只剩三件事：

  · **包加载口**：`bootstrap.package_apply()`（本进程唯一；幂等）
  · **全量再导出**：名字集合与改造前**逐名相同** → `game/core/__init__.py` 的
    `from .item_templates import …`、别线模块的 `from .item_templates import …`、测试的模块属性访问**零改动**
...
改造前 1233 行 → 现在 66 行（`TEMPLATES` / `META` 再导出的就是包内那两只注册表字典）。
"""
...
from content import item_templates as _IMPL                      # noqa: E402  包内唯一实现
...
_LEGACY_DATA_NAMES = frozenset(["TIPS"])
