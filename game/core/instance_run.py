# -*- coding: utf-8 -*-
"""奥兰迪亚·余烬纪年 核心层 - instance_run.py（v185：副本运行态 → 引擎 run 形状适配层）

**这一层做什么**：副本运行态 `st`（存在 `battle.state` 里的存档 dict）里的
「谁在里面」（名单）与「打到哪了」（进度 / 剩余池 / 资源预算），拿到引擎形状
`saintess_engine.run.Roster` / `Progress` 上跑，**只此一份实现**。

**存档红线（勿破）**：`st` 的键名、类型、缺失语义**一律不变** —— 本模块是
「读 → 构造视图 → 操作 → 写回」，不是替换存档格式。老存档（缺字段）继续照旧可用。

对应关系：

| st 字段 | 引擎形状 | 说明 |
|---|---|---|
| `members` / `alive` / `leader` | `Roster` | 保序成员 + 存活（缺键 = 存活）+ 队长 |
| `inst_stages` / `stage_idx` / `stage_pending` | `Progress`（分层形态） | 每层一个节点，池 `units` = 该层待清队列 |
| `rooms[sa].monsters_left` / `pois_left` / `boss_alive` / `_is_boss` | `Progress`（房间形态） | 每房间一个节点，池 `monsters` / `pois` |
| `resources_pool`（gold_left / mats_left / equip_left） | `Progress` 预算 | 「不足只给剩余」语义 |

旧调用点若直接改 `st` 的字段（`st["members"].sort(...)`、`pool.pop(0)`、
`_pois_left.remove(...)`），一律改走本模块的函数 —— 于是这些语义只有一个来源。
"""
...
def current_members(group_id: str, st: dict) -> list:
    ...
    """
    from .. import db
    party = [str(m) for m in db.party_members(group_id, st.get("leader"))]
    r = roster_of(st)
    if party:
        return r.only(party)
    members = r.members
