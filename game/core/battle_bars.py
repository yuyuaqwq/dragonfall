...
def _state_prefix() -> str:
    """条状态在 effects 容器里的键前缀（声明源 = data/battle2_rules）。"""
    import importlib
    try:
        from ..data.battle2_rules import BAR_STATE_PREFIX  # noqa: F401
        return BAR_STATE_PREFIX or "bar:"
    except Exception:
        pass
    try:
        bc = importlib.import_module("game.data.battle2_rules")
        return getattr(bc, "BAR_STATE_PREFIX", "bar:") or "bar:"
    except Exception:
        return "bar:"
