# -*- coding: utf-8 -*-
"""game/core/quality_tiers.py —— B13-L1 **薄壳**（2026-09-14）

真源（**唯一实现**）已搬进内容包：`framework/games/orlandia/content/quality_tiers.py`
（搬的边界 / 正文改动面 / 缺口全写在那边的头注里）。本文件只剩三件事：

  · **包加载口**：`bootstrap.package_apply()`（本进程唯一；幂等）
  · **全量再导出**：名字集合与改造前**逐名相同** → `game/core/__init__.py` 的
    `from .quality_tiers import …`、别线模块的 `from .quality_tiers import …`、测试的模块属性访问**零改动**
...
改造前 40 行 → 现在 66 行（本文件**刻意不含 `TierTable(` 字面**：`tests/test_v184_loot_tiers.py:671` 扫 `<插件>/game/**` 要求「全仓只有一处建档位表」= 包内那份）。
"""
...
from content import quality_tiers as _IMPL                      # noqa: E402  包内唯一实现
...
_LEGACY_DATA_NAMES = frozenset(["QUALITY", "QUALITY_ORDER", "QUALITY_CN", "FISH_QUALITY_WEIGHTS"])
