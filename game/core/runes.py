# -*- coding: utf-8 -*-
"""game/core/runes.py —— B13-L1 **薄壳**（2026-09-14）

真源（**唯一实现**）已搬进内容包：`framework/games/orlandia/content/runes.py`
（搬的边界 / 正文改动面 / 缺口全写在那边的头注里）。本文件只剩三件事：

  · **包加载口**：`bootstrap.package_apply()`（本进程唯一；幂等）
  · **全量再导出**：名字集合与改造前**逐名相同** → `game/core/__init__.py` 的
    `from .runes import …`、别线模块的 `from .runes import …`、测试的模块属性访问**零改动**
...
改造前 52 行 → 现在 66 行（`game/data/__init__.py` / `game/core/__init__.py` 的再导出链不断）。
"""
...
from content import runes as _IMPL                      # noqa: E402  包内唯一实现
...
_LEGACY_DATA_NAMES = frozenset(["RUNES", "RUNE_CONFLICTS", "RUNE_LEVEL_ROMAN", "QUALITY"])
