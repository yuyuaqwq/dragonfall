# -*- coding: utf-8 -*-
"""game/core/gems.py —— B13-L1 **薄壳**（2026-09-14）

真源（**唯一实现**）已搬进内容包：`framework/games/orlandia/content/gems.py`
（搬的边界 / 正文改动面 / 缺口全写在那边的头注里）。本文件只剩三件事：

  · **包加载口**：`bootstrap.package_apply()`（本进程唯一；幂等）
  · **全量再导出**：名字集合与改造前**逐名相同** → `game/core/__init__.py` 的
    `from .gems import …`、别线模块的 `from .gems import …`、测试的模块属性访问**零改动**
...
改造前 163 行 → 现在 68 行（`random` 再导出 = 包内同一只 random 模块（随机序列可被测试打点））。
"""
...
from content import gems as _IMPL                      # noqa: E402  包内唯一实现
...
_LEGACY_DATA_NAMES = frozenset(["GEM_TIERS", "GEM_TIER_NAMES", "GEM_STATS", "GEM_SOCKETS", "GEM_REMOVE_COST",
                              "GEM_LEGENDARY_EFFECTS", "GEM_DROP_RATE", "GEM_DROP_TIER",
                              "GEM_BOSS_FIXED", "GEM_ITEM_TYPE"])
