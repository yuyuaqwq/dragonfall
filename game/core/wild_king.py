...
- 全服同一时刻每图至多 1 只、全服 ≤3 只（WILD_KING_GLOBAL_LIMIT）
...
⚠️ 不模块级 import db——data/_assembly 加载会循环导入（同 wild.py 铁律）。
所有用 db 的函数内延迟导入。
"""
import datetime
import json
...
from ..data.wild_king_data import (
    ...
    WILD_KING_PITY_PERIODS, WILD_KING_NO_KILL_EXTRA,
)
...
def build_king_monster(king: dict, map_obj: dict, player: dict) -> dict:
    ...
    """
    from ..core.drops import build_monster  # noqa: E402（延迟导入防循环）
...
def _resolve_map_name(map_id: str) -> str:
    from ..data.maps import MAP_BY_ID  # noqa: E402
