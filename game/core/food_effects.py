...
def _f_h_bleed(battle, player, dmg, logs):
    ...
    if random.random() < float(_fp("bleed", "chance", 0.20)):
        ...
        action_dot(battle, logs, key="bleed", stacks=int(_fp("bleed", "stacks", 3)), max_n=int(_fp("bleed", "max_n", 3)), target=battle._hit_tgt())  # 烬火辣椒 3 层
...
def _f_h_armor_break(battle, player, dmg, logs):
    ...
    if random.random() < float(_fp("armor_break", "chance", 0.25)):
        ...
        action_def_down(battle, logs, turns=int(_fp("armor_break", "turns", 2)), pct=float(_fp("armor_break", "pct", 0.15)), target=battle._hit_tgt())
...
def _f_h_combo(battle, player, dmg, logs):
    ...
    if random.random() < float(_fp("combo", "chance", 0.15)):
        ...
        action_bonus_pct(battle, player, dmg, logs, pct=float(_fp("combo", "pct", 0.50)), tag="⚡", name="连击", target=battle._hit_tgt())
...
def _f_h_element_fire(battle, player, dmg, logs):
    ...
    action_element_dmg(battle, player, dmg, logs, pct=float(_fp("element_fire", "pct", 0.05)), tag="🔥", name="火焰附加", target=battle._hit_tgt())
...
def _f_h_element_ice(battle, player, dmg, logs):
    ...
    action_element_dmg(battle, player, dmg, logs, pct=float(_fp("element_ice", "pct", 0.05)), tag="❄️", name="冰霜附加",
                       slow_turns=int(_fp("element_ice", "slow_turns", 2)), target=battle._hit_tgt())
...
def _f_h_pierce(battle, player, dmg, logs):
    ...
    if random.random() < float(_fp("pierce", "chance", 0.20)):
        ...
        action_pierce_dmg(battle, player, logs, atk_pct=float(_fp("pierce", "atk_pct", 0.60)), target=battle._hit_tgt())
...
def _f_h_charge(battle, player, dmg, logs):
    ...
    if random.random() < float(_fp("charge", "chance", 0.10)):
        from .effect_actions import action_bonus_pct
        action_bonus_pct(battle, player, dmg, logs, pct=float(_fp("charge", "pct", 0.50)), tag="💪", name="蓄力爆发", target=battle._hit_tgt())


@register(FOOD_HIT_EFFECTS, "static")
def _f_h_static(battle, player, dmg, logs):
    """雷雨藤烤串（v102.3）：静电麻痹——攻击 20% 令敌方减速（2 刻）"""
    if random.random() < float(_fp("static", "chance", 0.20)) and battle._hit_tgt().get("hp", 0) > 0:
        ...
        logs.append(f"⚡ 静电麻痹！【{battle._hit_tgt().get('name', '敌人')}】速度下降！")
...
def _f_t_counter(battle, player, ctx, logs):
    """狼肉干：20% 反击 60% 伤害"""
    if random.random() < float(_fp("counter", "chance", 0.20)) and battle._hit_tgt().get("hp", 0) > 0:
        from .effect_actions import action_counter
        action_counter(battle, player, logs, atk_pct=float(_fp("counter", "atk_pct", 0.60)), target=battle._hit_tgt())


@register(FOOD_TAKEN_EFFECTS, "thorns")
def _f_t_thorns(battle, player, ctx, logs):
    """鹿奶干酪：10% 反弹 30% 伤害（基于原始 dmg）"""
    if random.random() < float(_fp("thorns", "chance", 0.10)) and battle._hit_tgt().get("hp", 0) > 0:
