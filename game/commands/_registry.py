...
COMMAND_REGEX = {
    ...
    "weekly_cmd": r'^(?:\[At:\d+\]\s*)?周常(?!列表)(?:\s*|$)',
    "weekly_list": r'^(?:\[At:\d+\]\s*)?周常列表(?:\s+(\d+))?\s*$',
    "defend": r'^(?:\[At:\d+\]\s*)?防御(?:\s*|$)',
    "enchant": r'^(?:\[At:\d+\]\s*)?附魔(?:\s*|$)',
    "encyclopedia": r'^(?:\[At:\d+\]\s*)?百科(?:\s*|$)',
    "monster": r'^(?:\[At:\d+\]\s*)?怪物(?:\s+(\S+))?\s*$',
    "enhance": r'^(?:\[At:\d+\]\s*)?强化(?:\s*|$)',
    # v135 装备升级（养装备，与强化同副业门槛）
    "equip_upgrade": r'^(?:\[At:\d+\]\s*)?升级(?:[\s\S]*)$',
    # v136 原石系统（打孔/镶嵌/拆卸/合成/查看）
    "gem_drill": r'^(?:\[At:\d+\]\s*)?打孔(?:[\s\S]*)$',
    "gem_socket": r'^(?:\[At:\d+\]\s*)?镶嵌(?:[\s\S]*)$',
    "gem_remove": r'^(?:\[At:\d+\]\s*)?拆卸(?:[\s\S]*)$',
    "gem_combine": r'^(?:\[At:\d+\]\s*)?原石合成(?:[\s\S]*)$',
    "gem_view": r'^(?:\[At:\d+\]\s*)?原石(?!合成)(?:\s*|$)',
    # v136 Phase 3 符文可制作+可拆卸（独立命令，与『拆卸』原石命令并行不冲突）
    "rune_craft": r'^(?:\[At:\d+\]\s*)?符文制作(?:\s*|$)',
    "rune_remove": r'^(?:\[At:\d+\]\s*)?符文拆卸(?:\s*|$)',
    # v136 Phase 4 装备进化（怪猎派生树）：『装备进化』与『转职』(evolve) 不冲突
    "evolve_equip": r'^(?:\[At:\d+\]\s*)?装备进化(?:[\s\S]*)$',
    # v136 Phase 5 怪异炼成（怪猎曙光怪异化）：『炼成』每件限 3 次
    "calamity_forge": r'^(?:\[At:\d+\]\s*)?炼成(?:[\s\S]*)$',
    "equip": r'^(?:\[At:\d+\]\s*)?(?:装备(?!进化)|我的装备)(?:\s*|$)',
    ...
    "item_detail": r'^(?:\[At:\d+\]\s*)?(?:物品详情|查看(?!地图|任务|背包|技能|图鉴|百科|成就|称号|宠物|副本|排行|帮助|列表|商店|位置|声望|荣誉|套装|坐骑|队伍|摊位|市场|仓库|事件|许愿|种族|流派|签到|战力|职业|配方|烹饪|副业|锻造|炼金|合成|钓鱼|采集|挖掘|学习|洗点|装备|卸下|喂养|放生|传送|祭坛|赶路|深入|调查|撤退|离开|注册|意见|怪物|咨询|状态|属性|详情|信息))(?:[\s\S]*)$',
    ...
    "region_view": r'^(?:\[At:\d+\]\s*)?区域(?:\s*|$)',
    # v128.2: 『位置 0』/『位置0』旧捷径已移除——正则捕获 0 后缀仅为面板提示『赶路』入口，不再切换赶路模式
    "location_view": r'^(?:\[At:\d+\]\s*)?位置(?:\s*0)?(?:\s*|$)',
    ...
    "stall_deprecated": r'^(?:\[At:\d+\]\s*)?摆摊(?:[\s\S]*)$',
    "stall_sell": r'^(?:\[At:\d+\]\s*)?摆卖(?:[\s\S]*)$',
    "stall_exchange_pawn": r'^(?:\[At:\d+\]\s*)?摆换(?:[\s\S]*)$',
    ...
    "recipe_list": r'^(?:\[At:\d+\]\s*)?(?:配方|图纸列表)(?:\s*|$)',
    ...
    "wild_king_chest": r'^(?:\[At:\d+\]\s*)?摸(?:战利箱|宝箱)(?:\s*|$)',
