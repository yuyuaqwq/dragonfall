...
COMMAND_REGEX = {
    ...
    "guild_info": r'^(?:\[At:\d+\]\s*)?公会(?!签到|任务|捐献|排行|创建|加入|退出|解散|商店|技能|任命|免职)(?:\s*.*|$)',
    ...
    # v116 公会成长纵深（职位/商店/技能）：与 social.py @filter.regex 1:1 同步
    "guild_shop": r'^(?:\[At:\d+\]\s*)?公会商店(?:\s*.*|$)',
    "guild_skill_view": r'^(?:\[At:\d+\]\s*)?公会技能(?:\s*.*|$)',
    "guild_appoint": r'^(?:\[At:\d+\]\s*)?公会任命(?:\s*.*|$)',
    "guild_demote": r'^(?:\[At:\d+\]\s*)?公会免职(?:\s*.*|$)',
    # v116 阵营国战最小闭环（四阵营/任务/商店/排行）：与 world.py @filter.regex 1:1 同步
    "camp_join": r'^(?:\[At:\d+\]\s*)?加入阵营(?:\s+\S+)?$',
    "camp_task": r'^(?:\[At:\d+\]\s*)?阵营任务(?:\s+\S+)?$',
    "camp_shop": r'^(?:\[At:\d+\]\s*)?阵营商店(?:\s+\S+)?$',
    "camp_rank": r'^(?:\[At:\d+\]\s*)?阵营排行(?:\s*|$)',
    ...
    "shortcut_trigger": r'^(?:\[At:\d+\]\s*)?(?!gm_)(?!help(?:\s|$))(?![+＋\-－＝=@\[/])(?:[0-9０-９]\d*|[a-zA-Z][a-zA-Z0-9]*|[^\s0-9０-９a-zA-Z\u4e00-\u9fff])[\s\S]*$',
    ...
    "trader_confirm": r'^(?:\[At:\d+\]\s*)?(?:确认购买|拒绝)(?:\s*|$)',
    # O119：战败结算复活羽毛二段回复（消耗羽毛免扣金币 / 放弃复活损失金币）
    "revive_confirm": r'^(?:\[At:\d+\]\s*)?(?:使用复活羽毛|放弃复活)(?:\s*|$)',
