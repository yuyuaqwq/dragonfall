# -*- coding: utf-8 -*-
"""奥兰迪亚·余烬纪年数据层 - set_bonus_data.py

10 章五节名册套装效果表（v181-P2A 自 game/core/class_sets.py 原值下沉，纯搬移零逻辑）。

表内容说明（随 class_sets.py 装配器 _build_class_sets 消费）：
- 9 大系列（橡木/铁港/圣光/月语/霜狼/龙脊/海神/地底/苍穹/白鹿/银铃/翡翠/迷雾/
  星尘/灰烬守卫）+ v136 Phase6 职业套（18）+ 区域套（5）= 本表全部 38 条。
- bonus_2：2 件属性百分比（引擎 set_bonus_2 消费；含 v110 圣光 heal_power 审计修复）
- piece_heal_power：v181-A1 圣光套任意件数持有加成（battle 治疗段泛读）
- bonus_4_stats：4 件属性百分比（engine.set_bonus_2 叠加，>=4 件生效）
- bonus_4.effect：4 件特效（战斗触发型，保留旧结构兼容）
- bonus_3/bonus_3_stats：v136 Phase6 区域套 3 件特效/属性
- bonus_5：5 件效果（v104 起战斗落地：battle 对敌增伤/元素增伤、battle_mech 抗寒）
- bonus_5_ctrl_immune：v180-B ② 5 件套控制免疫数据化（如霜狼抗寒）
- bonus_5_cond：v126 数值下沉 5 件对敌增伤条件（敌方名字关键词/player_hp_below/dmg_mult/tag）
- class：v136 Phase6 职业套装归属（本职业 100% / 非本职业 60% 职业折扣）

⚠️ 历史注记（随表迁移保留）：
- v110 审计修复：圣光套 bonus_2 键原为 "heal"（heal ∉ STAT_NAMES）→ engine 属性结算
...
- v181-A1：5 件元素增伤数据声明（月语/海神/苍穹 element_dmg，替代 battle 按中文套装名特判）
- v181-A1：星尘 5 件夜间回蓝数据化（effect=night_mp_regen + params）
- v180-B ②：霜狼 5 件套控制免疫数据化（bonus_5_ctrl_immune=["slow"]）
- v126 数值下沉：5 件对敌增伤条件（敌方名字关键词）从数据声明，battle 查表消费
"""

# ===== v181-P2A：套装数据表自 game/core/class_sets.py L20-157 原值搬移（数值零变化）=====
SERIES_SET_BONUS = {
    "橡木": {
        "icon": "🌳", "quality": "white",
    ...
    '渡口': {'icon': '⛵', 'quality': 'blue', 'bonus_2': {'spd': 0.06}, 'bonus_3_stats': {'mdef': 0.06}, 'bonus_3': {'effect': 'ferry_repel', 'chance': 0.25, 'params': {'type': 'taken_counter', 'chance': 0.25, 'atk_pct': 0.40, 'once_per_round': True, 'tag': '🌊', 'name': '渡口回潮'}, 'desc': '被攻击命中后 25% 概率反击 40% 攻击力（每刻最多 1 次）'}}, 'bonus_5_cond': {'enemy_contains': ["章鱼", "乌贼", "海蟹", "鲨", "鲸", "雷鳗", "水母"], 'dmg_mult': 1.10, 'tag': '⛵渡口守潮'},
    '巡林': {'icon': '🌲', 'quality': 'blue', 'bonus_2': {'spd': 0.04, 'dodge': 0.03}, 'bonus_3_stats': {'spd': 0.05}, 'bonus_3': {'effect': 'ranger_net', 'chance': 0.3, 'params': {'type': "proc_slow", "chance": 0.3, "slow_pct": 0.15, "slow_turns": 2, "tag": "🕸️", "name": "巡林罗网"}, 'desc': '攻击命中 30% 概率使敌方速度 -15%（2 刻）'}}, 'bonus_5_cond': {'enemy_contains': ["精灵", "仙灵", "妖精", "花妖"], 'dmg_mult': 1.10, 'tag': '🌲巡林逐灵'},
    '霜猎': {'icon': '🐺', 'quality': 'purple', 'bonus_2': {'crit': 0.05, 'atk': 0.03}, 'bonus_3_stats': {'atk': 0.05}, 'bonus_3': {'effect': 'frost_hunt_freeze', 'chance': 0.2, 'params': {'type': "proc_freeze", "chance": 0.2, "freeze_turns": 1, "tag": "🧊", "name": "霜猎冰冻"}, 'desc': '攻击命中 20% 概率冻结敌人 1 刻（Boss 退化为减速 40%）'}}, 'bonus_5_cond': {'enemy_contains': ["雪狼", "冰狼", "极地", "冰川", "雪兔"], 'dmg_mult': 1.10, 'tag': '🏹霜猎夺心'},
    '龙裔': {'icon': '🐉', 'quality': 'purple', 'bonus_2': {'mdef': 0.05}, 'bonus_3_stats': {'atk': 0.06}, 'bonus_3': {'effect': 'long_yi_dread', 'chance': 0.3, 'params': {'type': "proc_mon_atk_down", "chance": 0.3, "atk_down_pct": 0.15, "turns": 2, "tag": "🐉", "name": "龙威压制"}, 'desc': '攻击命中 30% 概率使敌人攻击 -15%（2 刻）'}}, 'bonus_5_cond': {'enemy_contains': ["风暴", "雷霆", "雷暴", "天鹰"], 'dmg_mult': 1.10, 'tag': '🐉龙裔镇风'},
}
