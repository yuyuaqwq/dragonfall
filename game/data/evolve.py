# -*- coding: utf-8 -*-
"""奥兰迪亚·余烬纪年数据层 - evolve.py（v136 装备进化，怪猎派生树）

同系列旧武器 + 稀有素材 + 金钱 → 同系列高阶武器，继承强化/升级等级（投资不沉没）。
派生树：同一基础武器可派生多个方向（玩家做选择）。

首批每系列 1 条进化链（v136 定稿，后续扩展）：
- 弯刀(Lv14 蓝) → 血誓战剑(Lv36 紫)      铁港→血誓 系列（新手第一把进化武器）
- 血誓战剑(Lv36 紫) → 余烬军团战剑(Lv55 紫)  血誓→余烬军团 系列
- 日冕权杖(Lv56 紫) → 夜祷权杖(Lv66 紫)  圣典·日冕→暗夜圣典 系列（牧师武器进化）

inherit: "full"=完整继承强化/升级等级；"half"=折半向下取整（v136 定稿）
mats: 稀有素材（进化消耗，额外加 Boss 怨念物/稀有矿）
"""
EVOLVE_RECIPES = {
