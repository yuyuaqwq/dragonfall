...
#   ✅ 新值独立分档（V3，真实玩家模型验证）：≤15 ×1.2 / 16-30 ×2.0 / 31-60 ×1.0 / 61+ ×0.7
#      → 全程精英 ≈ Boss 的 2~3 成（Boss 是精英的 3~5 倍，无"精英>Boss"反直觉）；
#      → Lv24 精英 9,688（Boss 29%）；Lv10 精英 1,216（新手可挑战）；Lv60 精英 33,588（Boss 32%）。
#   验证（numeric_lib 真实玩家蓝+5 单刷轮次）：精英 2~23 轮（法师 2~13 / 战士 4~23 / 游侠 3~20），
#   对比 Boss 45~80 轮——精英明显轻松；31/61 档位边界跟随 Boss 档位（×2.7→×1.5→×1.1），占比平滑无断层。
FIELD_TIER_MULT = {
    "elite": ((15, 1.2), (30, 2.0), (60, 1.0), (999, 0.7)),  # ≤15 ×1.2 / 16-30 ×2.0 / 31-60 ×1.0 / 61+ ×0.7
    "boss":  ((30, 2.7), (60, 1.5), (999, 1.1)),    # v131.1 单刷化标定：≤30 ×2.7 / 31-60 ×1.5 / 61+ ×1.1
