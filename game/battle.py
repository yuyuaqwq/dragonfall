...
class Battle:
        ...
        if _kind == "skill":
            ...
            if not sinfo:
                ...
                try:
                    ...
                    if _rg_s and isinstance(_rg_s, dict):
                        for _rk_s, _rv_s in _rg_s.items():
                            self._res_gain(e, _rk_s, int(_rv_s or 0), logs)
                except Exception:
                    pass
                # v178 E4：怪物技能带 pdot（给目标挂持续伤害：毒/灼烧/流血/腐蚀）
                ...
                # ⚠️ v178 重构：挂 dot = _apply_dot(target=玩家, source=怪)——玩家/怪同入口
                try:
                    ...
                    if _pdot and isinstance(_pdot, dict) and not self.p_buffs.get("cc_immune"):
                        _pdt = self._apply_dot(player, e, _pdot, logs)
                except Exception:
                    pass
    ...
    def _damage_actor(self, actor: dict, dmg: int, logs: list, source: str = "伤害",
        ...
        if _is_player:
            ...
            HITS = self._p_buff_hits
            SH = self.p_shields
            TARGET = self.enemy  # 玩家被打时，反击/反伤目标是敌人
            TARGET_STATS = self._enemy_stats
            RL_ALL = self._reduce_all_left
            RL = self._reduce_left
        else:
            B = actor.setdefault("buffs", {})
            EFF = actor.get("eff") or {}
            RES = actor.setdefault("resources", {})
        ...
        if CH and CH.get("skill"):
            ...
            cname = CH.get("name", CH.get("skill", "?"))
            spent = int(CH.get("mp_spent", 0) or 0)
            # v177 actor 化：局部别名赋值无效，真清状态容器（玩家 self.charging / 怪物 actor["charging"]）
            if _is_player:
                self.charging = None
            else:
                actor["charging"] = None
