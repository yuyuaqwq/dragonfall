# -*- coding: utf-8 -*-
"""平台 API 面（**P5A 原样迁**：`game/commands/_platform.py` 逐字搬入 `host/`）。

迁移口径（P5A 作业书 §1「能迁的现文件原样迁，别重写一遍」）：
  · 下方分隔标记之后的正文与源件**逐字节相同**（源件 sha256 = 069f26afce4e22cf4af5b3f654e3e6362328fa8ea1dc65914def3afc50f86238，10802 bytes）；
  · 本模块 = 宿主**平台面**（AstrMessageEvent / MessageChain / 组件 / filter / 注册表），
    平台件、零游戏知识；P5C 删掉 `game/commands/**` 后本模块即唯一平台面。
"""
# ===== 以下为迁移正文（未改一字）=====
# -*- coding: utf-8 -*-
