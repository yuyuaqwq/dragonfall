...
def attack_skills(cls):
    """该职业所有攻击技能（kind ∈ 物理/魔法/真伤），返回 [(中文名, info)]"""
    out = []
...
def peak_of(cls, lv, loadout, diff, seeds=25):
    p = player_dict(cls, lv, loadout)
    skills = attack_skills(cls)
    tlv = lv + diff
...
def main():
    ...
    rows = peak_of(a.cls, a.lv, a.loadout, a.diff)
