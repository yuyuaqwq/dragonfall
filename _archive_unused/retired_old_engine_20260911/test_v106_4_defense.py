...
async def main():
    ...
    for seed in range(60):
        ...
        logs6b = b6b._actor_skill(b6b._player_stats(p6b), "火球术",
                                   E.skill_info("法师", "火球术"), p6b)
        if any("吸血" in l for l in logs6b):
            found6 = True
            break
    # v158 formula 补全：火球术等元素后缀技能配了 formula（type=magi 段），
    # _magi_part 分账正确 → 法吸魔法技能**生效**（v153 旧 bug 已随 formula 修复）。
    check("法吸魔法技能触发（v158 formula 修复后生效）", found6, f"found6={found6}")
