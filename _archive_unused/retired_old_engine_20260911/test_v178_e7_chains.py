...
random.seed(7)
# 首次行动
logs1, _ = b._actor_auto_turn(player)
sk1 = (mon.get("_last_skill_key"))
b._now += 1.0  # 下一行动刻
logs2, _ = b._actor_auto_turn(player)
sk2 = mon.get("_last_skill_key")
b._now += 1.0
logs3, _ = b._actor_auto_turn(player)
...
# 2. 链走完回绕（第4招回第1招）
b._now += 1.0
logs4, _ = b._actor_auto_turn(player)
...
b2._focus = mk_player()
random.seed(3)
b2._actor_auto_turn(player)  # 招1
b2._now += 1.0
b2._actor_auto_turn(player)  # 招2（链打完，设冷却 until = r+2）
_until = mon2.get("_chain_until")
check("链尾设置冷却", _until is not None and _until > 0, f"until={_until}")
b2._now += 1.0
# 冷却中 → 不出链招（回落随机池或普攻）
b2._actor_auto_turn(player)
...
b4._focus = mk_player()
random.seed(11)
logs5, _ = b4._actor_auto_turn(player)
