...
MECH_RULES = [
    ...
    (r'战意 ≥(\d+) 时暴击 \+(\d+)%', {"passive": {"proc": "zhan_yi_crit"}}, None),
    ...
    (r'战意满 (\d+) 时减伤 \+(\d+)%、免疫眩晕', {"passive": {"proc": "zhan_yi_full_reduce"}}, None),
    (r'被控时消耗 (\d+) 层战意跳过（每场 (\d+) 次）', {"passive": {"proc": "tenacity"}, "mech_val": 2}, None),
    ...
    (r'狂暴中生命首次归零时，清空战意复活并回 (\d+)%', {"passive": {"proc": "berserk_revive"}}, None),
    (r'进入狂暴时自动追加（不占行动）', {"auto": "fury_append"}, None),
    (r'守护姿态下首次致命伤免疫，清空全部战意', {"passive": {"proc": "stance_immortal"}}, None),
    (r'全队攻击 \+(\d+)% 持续 (\d+) 刻', {"effect": "atk_all"}, None),
    (r'每层战意额外 \+([\d.]+)% 吸血', {"passive": {"proc": "zhan_yi_lifesteal"}}, None),
    ...
    (r'三系同时 ≥(\d+) 层时，结算伤害 \+(\d+)%（\*\*加算\*\*，不是乘算）', {"passive": {"proc": "element_origin"}}, None),
    ...
    (r'每刻自动 \+(\d+) 充能（冥想中 \+(\d+)）', {"passive": {"proc": "arcane_intuition"}, "rate": 1, "meditate_rate": 2}, None),
    ...
    (r'奥术技能伤害 \+(\d+)%', {"passive": {"proc": "arcane_resonance"}}, None),
    ...
    (r'充能满 (\d+) 时，奥术暴击 \+(\d+)%', {"passive": {"proc": "arcane_wisdom"}}, None),
    (r'奥术技能耗蓝 −(\d+)%（能量循环）', {"passive": {"proc": "arcane_constant"}}, None),
    ...
    (r'猎印每层增伤 \+(\d+)%（叠加基础 8%）', {"passive": {"proc": "hunt_mark_up"}}, None),
    (r'猎印上限 \+(\d+) 层（至 (\d+) 层）', {"passive": {"proc": "hunt_mark_cap"}}, None),
    (r'对猎印目标 ×([\d.]+) \+ 全队暴击 \+(\d+)% 持续 (\d+) 刻', {"cond": {"type": "enemy_hunt_mark", "mult": None}, "effect": "crit_all"}, None),
    (r'毒层上限 \+(\d+)，毒爆增伤 \+(\d+)%', {"passive": {"proc": "poison_cap_up"}}, None),
    ...
    (r'召唤物存活时，专注充能 \+(\d+)/s（至 (\d+)/s）', {"passive": {"proc": "focus_regen_summon"}}, None),
    ...
    (r'结余 ≥(\d+) 时，下次技能暴击 \+(\d+)%（强化结余奖励）', {"passive": {"proc": "focus_surplus_crit"}}, None),
    ...
    (r'治疗溢出量的 (\d+)% 转为护盾', {"passive": {"proc": "heal_overflow_shield"}}, None),
    ...
    (r'治疗时，目标承伤的 (\d+)% 转移给自己（分担）', {"passive": {"proc": "faith_share"}}, None),
    ...
    (r'过载时不再力竭，改为全队回血 \+(\d+)%', {"passive": {"proc": "faith_overload_heal"}}, None),
    ...
    (r'骷髅上限 \+(\d+)（至 (\d+)）', {"passive": {"proc": "skeleton_cap"}}, None),
    (r'场上每只亡灵 每刻 \+([\d.]+) 信念', {"passive": {"proc": "undead_faith"}}, None),
    (r'信念 ≥(\d+) 时，致命伤由 1 只骷髅代为承受', {"passive": {"proc": "death_contract"}}, None),
    ...
    (r'灵魂标记上限 \+(\d+)（至 (\d+)），每层 \+(\d+)%', {"passive": {"proc": "soul_mark_cap"}}, None),
    ...
    (r'断连时只掉 (\d+) 段（而非减半）', {"passive": {"proc": "lian_duan_soft"}}, None),
    ...
    (r'影舞态内速度 \+(\d+)%、暴伤 \+(\d+)%', {"passive": {"proc": "shadow_dance_bonus"}}, None),
    ...
    (r'影舞态中所有技能 CD −(\d+)%', {"passive": {"proc": "shadow_dance_cd"}}, None),
    (r'连段满 (\d+) 时 ×([\d.]+)', {"cond": {"type": "player_mech_stacks", "mech": "lian_duan", "stacks": None, "mult": None}}, None),
    (r'终结技系数 \+(\d+)%（每段 10% → 16%）', {"passive": {"proc": "finisher_up"}, "per_stack": None}, None),
    ...
    (r'毒爆伤害 \+(\d+)%', {"passive": {"proc": "poison_burst_up"}}, None),
    ...
    (r'毒层上限 \+(\d+)（至 (\d+) 层）', {"passive": {"proc": "poison_cap"}}, None),
    (r'\[AOE\] 全体 \+ 毒 (\d+) 层持续 (\d+) 刻', {"aoe": "all", "mech": "poison", "mech_val": 2}, None),
    (r'毒层 ≥(\d+) 时，目标减速 (\d+)%、降防 (\d+)%', {"passive": {"proc": "poison_weaken"}}, None),
    ...
    (r'全部毒层伤害 \+(\d+)%', {"passive": {"proc": "poison_all_up"}}, None),
    ...
    (r'毒爆击杀目标时，毒层扩散给相邻敌人', {"passive": {"proc": "poison_spread"}}, None),
    ...
    (r'破防持续 \+([\d.]+) 刻，全队增伤 \+(\d+)%', {"passive": {"proc": "broken_extend"}}, None),
    (r'推条；伤害 = base ×\(1 \+ ([\d.]+) × 磐核数\)', {"shaken_gain": 5, "cond": {"type": "guard_core", "per_core": None}}, None),
    (r'磐核 ≥(\d+) 时，溢出承伤转为护盾', {"passive": {"proc": "core_overflow"}}, None),
    ...
    (r'全队减伤 (\d+)%，持续 (\d+) 刻', {"effect": "reduce_all"}, None),
    (r'受击时 (\d+)% 反击（普攻的 (\d+)%）', {"passive": {"proc": "counter_chance"}}, None),
    (r'受击时对攻击者反弹 (\d+)% 伤害 \+ 推条', {"passive": {"proc": "reflect"}, "shaken_gain": 3}, None),
    (r'反击概率 \+(\d+)%，反击伤害 \+(\d+)%', {"passive": {"proc": "counter_up"}}, None),
    (r'每核额外减伤 \+(\d+)%（与基础 \+(\d+)% 叠加）', {"passive": {"proc": "core_reduce"}}, None),
    ...
    (r'吟唱时强度额外 \+1 层', {"passive": {"proc": "melody_duet"}}, None),
    ...
    (r'强度层 ≥(\d+) 时，全队额外 \+(\d+)% 全属性', {"passive": {"proc": "melody_resonance"}}, None),
    ...
    (r'强度层满 (\d+) 时，全队额外 \+(\d+)% 全属性', {"passive": {"proc": "melody_full"}}, None),
    (r'每强度层 \+(\d+)% 旋律效果（\*\*满层 \+(\d+)%\*\*，基础为 \+(\d+)%）', {"passive": {"proc": "melody_master"}}, None),
    ...
    (r'挽歌系控制时长 \+([\d.]+) 刻', {"passive": {"proc": "dirge_ctrl_up"}}, None),
    ...
    (r'敌方每携带 1 个负面，受到伤害 \+(\d+)%（上限 \+(\d+)%）', {"passive": {"proc": "dirge_debuff_dmg"}}, None),
