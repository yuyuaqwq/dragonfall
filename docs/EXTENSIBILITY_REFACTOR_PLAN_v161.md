...
### 3.0 关键认知（2026-09-01 实测）

- **职业矩阵 DPS 锚点（32章 1.1）测的是分支技能轴（ROTATIONS）**：嗜血斩/蓄力射击/织焰/骨噬诅咒/链舞/崩拳（全部 T1 分支技能）
- **基础技能（挥砍等）不影响矩阵 DPS**——样板改战士+法师基础技能，stage_scan 数值零变化（实测 3 方案 DPS 全同）
- **职业成长系数补偿必须作用在分支技能轴**才能影响矩阵；基础技能是新手期过渡，补偿同样适用但影响面小
- 样板验证策略：基础技能迁移验证"引擎 exprs 正确性 + 展示效果"；分支技能迁移验证"矩阵 DPS 锚点 + 梯队"
