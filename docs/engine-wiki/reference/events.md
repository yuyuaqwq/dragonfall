...
|---|---|---|---|---|---|
| 1 | `battle_start` | `battle.py:477`（`_ensure_battle_started`） | `{}` | **无**（全体触发） | 首个 actor 行动前，**整场一次**（`_started` 守卫） |
| 2 | `turn_start` | `battle.py:378`（`act`） | `actor` | 行动者 | 回合开始，**先于控制检查**（所以「回合开始回蓝」被晕也触发） |
| 3 | `act_begin` | `battle.py:408`（`act`） | `actor`, `target` | 行动者 | 控制通过、行动执行前 |
...
| 7 | `crit` | `actions.py:419-420` | `actor`, `target`, `info`, `dmg` | 攻击者 | 暴击命中（`skill_hit`/`attack_hit` 的子集，**紧跟其后**） |
...
| 11 | `on_death` | `battle.py:495`（`_on_actor_dead`） | `actor`, `target` | 死者 | 所有死亡路径统一在此；**死者的声明仍会执行**（subject 例外） |
...
| 14 | `on_act_consume` | `battle.py:403`（`act`） | `actor`, `tag` | 行动者 | 被控跳过行动（`mode="skip"`）时 |
| 15 | `on_hit_consume` | `actions.py:464`（`_consume_hit_buffs`） | `actor`, `key` | 出手者 | 一次性出手 buff 被消费时 |
...
| 18 | `dmg_calc` | `actions.py:379`（`_single_target_pipeline`） | `actor`, `target`, `dmg`, `is_crit`, `info`, `mult` | 攻击者 | 伤害算出后、落地前（攻击方乘区） |
| 19 | `taken_calc` | `landing.py:79`（`deal_damage`） | `actor`, `target`, `source`, `dmg`, `mult` | 承伤者 | 承伤修正（承伤方乘区） |
| 20 | `heal_calc` | `actions.py:656`（`_do_heal`） | `actor`, `target`, `heal`, `info`, `mult` | 施法者 | 治疗量算出后、落地前 |
| 21 | `act_done` | `battle.py:442`（`act` 尾部） | `acted`（**不是** `actor`） | **无**（全员广播） | 行动完成；被控跳过不触发 |
...
（`actions.py:339-423` + `landing.py:23-143` + `battle.py:358-447`）
