...
`Battle.to_state` / `Battle.from_state` 是类方法包装（`battle.py:526/532`）。
...
| `act_count` | `actor_auto` 每动 +1（`battle.py:351`） | AI `round_mod` 谓词读它 |
...
`battle_start` 事件是**整场一次**的（`_ensure_battle_started`，`battle.py:466`），
...
`killed_actors`，`battle.py:481-497`；`_check_side_end` 也不删）。
