...
## 10. v1.1 增补：混合伤害公式 + 机制特色（用户拍板）

用户反馈：纯百分比（毒 5%/层）打血牛 Boss 太强、纯固定值打后期 Boss 太弱；各 dot 需要机制差异化，不能只是换皮数值。
**v1.1 覆盖 §2.2 的伤害公式**：从 `max_hp×pct×n` 改为混合公式；并为每种 dot 增加独立特色。架构（目标级 debuffs / 层数衰减 / dot_res 抗性 / 护盾生效 / 共享结算）不变。

### 10.1 混合公式（每层每回合）与特色

| 类型 | 公式（每层每回合） | 段位/减免 | 特色机制 |
|---|---|---|---|
| poison 毒 | `atk×0.5 + max_hp×1.5%` | magi（吃魔免，不吃元素抗） | **毒蚀**：每层使目标 def/mdef -4%（上限 -20%），随层数衰减自动恢复 |
| burn 灼烧 | `matk×0.4 + max_hp×1%` | magi（吃魔免 + 火元素抗） | **易燃**：灼爆（burn_burst）时层数≥3 → 引爆伤害 ×(1+10%×(n-2))（3层+10%/4层+20%/5层+30%）；引爆本体每层 30% matk 保留 |
| bleed 流血 | `atk×0.6 + max_hp×1.5%` | phys（食物免） | **放血**：目标当前生命 <30%（处决线）时伤害 ×2——与处决/斩杀联动 |
| mark 标记 | 无伤害 | — | **易伤修复**：`_apply_mark` 改为按层数每层 +20%（5 层 +100%，对齐设计 27章:305）；e_buffs["mark"] 2 回合计时保留 |

- `atk`/`matk` 取结算方玩家快照：`self._player_stats(player)["atk"]/["matk"]`。
- 总公式：`p = int((atk_part + max_hp_part) * mult * (1 - dot_res))`（mult=debuffs 里的被动倍率，作用于整体）。
- **毒爆（poison_burst）v1.1 修正**：伤害 = `atk×0.15×n`，**物理段吃 def**（与毒 dot 的 atk 口径一致，修复 battle_mech.py:343-359 用 matk 与 skills.py 注释"按 atk"的矛盾）。仍不吃 dot_res（爆发直伤）。
- 灼爆（burn_burst）保持 `matk×0.30×n` 魔法段。

### 10.2 数值验证（已计算）

| 场景 | 结果 |
|---|---|
| 前期 atk=100 / 怪 500 血 | 毒 1 层 = 50+7.5 ≈ 57.5 ≈ **11.5% 血/回合** |
| 中期精英 atk=800 / 10 万血 | 毒满层 = 5×(400+1500)=9500 ≈ 9.5% → 80% 抗 → **1.9%/回合**（10 回合 ≈ 19% 总伤） |
| 后期 Boss atk=3000 / 500 万血 | 毒满层 = 5×(1500+75000)=38.25 万 ≈ 7.65% → 90% 抗 → **0.77%/回合**（20 回合 ≈ 15% 总伤） |
| 小怪（0 抗）满层 | ≈ **32.5%/回合** → 2~3 回合毒死，清怪效率保持 |
| 纯 dot 杀 Boss | 不可能：0.77%/回合 × 层数窗口 5 回合，必须直伤/毒爆补刀 ✓ |

atk 部分保证前期 dot 不废（固定值对低血怪有存在感）、后期随玩家成长；max_hp 1~1.5% 部分保证对高血 Boss 有基础压制但跳不死。

### 10.3 特色实现要点

- **毒蚀**：`Battle._enemy_stats()` 返回的 est 中 `def`/`mdef` 按 `debuffs.poison.n × 4%`（上限 20%）动态扣减（层数衰减自动恢复，不改 enemy dict 本体）。叠毒日志追加 `🛡️ 毒蚀：目标防御 -{4*n}%`。
- **易燃**：仅作用于灼爆（burn_burst）——n≥3 时引爆伤害 ×(1+0.10×(n-2))。不作用于其他火系伤害（避免污染散落的火系路径）。
- **放血**：`_tick_dots` 内 bleed 分支：`if e.get("hp",0) < e.get("max_hp",1)*0.30: p *= 2`，日志标注 `(放血)`。
- **标记易伤**：`_apply_mark`（battle.py:2706-2709）改为 `dmg × (1 + 0.20 × n)`（n=debuffs["mark"]["n"]，上限 5 层 +100%）；e_buffs["mark"] 计时保留。

### 10.4 职责更新（叠加在 §9 上）

- **A（battle.py）**：`_tick_dots` 混合公式（atk/matk 快照 + mult 整体乘算）；毒蚀（`_enemy_stats` 修正）；放血（bleed <30% ×2）；`_apply_mark` 按层。
