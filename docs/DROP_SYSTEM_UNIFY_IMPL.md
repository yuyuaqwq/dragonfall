...
4. **精英专属表（ELITE_EQUIP_DROP）—— ⚠️ 死数据发现**
   - 全仓除 data 导出**无任何消费端引用**；精英实际掉装走 roll_drop_equip 通用池（source=精英专属 就近抽）
   - 即"哪个精英专属掉哪件"的登记从未生效——玩家精英战只掉通用池
   - 新引擎 elite: 池已建好（18 件），接消费端 = **功能开发**（让精英真掉专属），属后续版本
