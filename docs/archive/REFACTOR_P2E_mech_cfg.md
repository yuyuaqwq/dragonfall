...
- **收编进 MECH_CFG（机制单表）**：52 个被消费常量中**仍以"职业名"命名**的 CFG/标量（§3.4/§4.2：B 组 14 个职业化命名改机制键收编 + A 组 29 个已是机制键的通用表直接收编键名不变）。
...
### 3.4 已消费但命名职业化的常量 → 机制键去向表（16 个，逐行落位；B 组 14 个收编 + ZEN_HOLD_CFG 删 + ENEMY_BAR_CFG 键名不变归 A 组）
...
- **本批次交付**：本文档（docs/archive/REFACTOR_P2E_mech_cfg.md，commit 到 wt_p2e 分支）。未改 game/ 代码。
...
> 本附录由执行批次回写（任务书 docs/archive/REFACTOR_P2E_task.md §P3c.7）。**已随 wt_p2e merge 095abd4 完成 P1a-P3a**；P3b/P3c 仍在 wt_p3c 分支推进（battle.py 读点迁移 / tests·scripts·docs 收尾）。顶层旧常量名暂保留作兼容层（P3c 后评估移除）。
