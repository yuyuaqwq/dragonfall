...
> **⚠️ 方向修正（次日发现）**：权威对照源不是 v139 留档，而是 **docs/CLASS_MECHANICS_v153.md**（v153 现行职业机制设计：战士战意满 10 狂暴/法师印记+奥术充能/游侠流量预算/牧师信念负载/刺客连段/拳师破绽+磐核/诗人旋律驻留——含每职业技能数值表与机制详设）。
> v139 留档是旧签名字段参考（多数被 v151/v153 取代）；**真正的缺口 = v153 设计 vs 现网技能数据 vs battle2 实现 三方对照**（如：战士血祭/嗜血斩等狂战士线技能设计存在但现网 BRANCH 未落地；诗人旋律技能已落地但引擎零实现）。
> 结论先行：**P0 = melody 驻留旋律系统**（基础诗人 5 技能 + 歌者分支技能已上线但 battle2 零实现 = 空转白板）。
