...
> 排在 I3-I7 之前。方案 `docs/archive/REFACTOR_v181P4_EFFECTS_UNIFY_design.md`。
> 鱼鱼验收北极星：**最后清干净——N10 删旧 battle.py + 旧效果系统三套注册表，
> 不留兼容壳/开关/兜底参数**（鱼鱼原话："老的一定要确保删掉，最后不留东西"）。
> 会话重启口令：「继续 effects 统一，读 docs/archive/REFACTOR_v181P4_EFFECTS_UNIFY_design.md
> 从 V1 开始」
...
- 效果系统主方案：`docs/archive/DESIGN_effect_system_v2.md`（Part 3.3/4 定稿 19 时机 + 管线）
- N9 施工方案：`docs/archive/REFACTOR_v181P4_N9_migration.md`（盘点/批次/删除清单）
- N9A 缺口方案：`docs/archive/REFACTOR_v181P4_N9A_weapon_gap_plan.md`（weapon 缺口设计）
- N9.7 affix 方案：`docs/archive/REFACTOR_v181P4_N9_7_affix_migration.md`（分档 + 进度）
- **N5b-4 命令层切换详细设计（下一步主工程，字段/函数级施工图）**：
  `docs/archive/REFACTOR_v181P4_N5B4_command_switch.md`
...
7. **（2026-09-08 N5b4-4）PVP 切 battle2（本 commit）**
   ...
   产出）**：docs/archive/REFACTOR_v181P4_N5B5a_instance_mainline.md——命令注册约束（同名
   『攻击』无法共存→CombatCmds 仅 4 处接线点改调用目标）、instance_router 新文件
   职责、instance.py 删除清单、R1-R5 分步。**新会话口令：「继续 N5b4-5a v3，读
   docs/archive/REFACTOR_v181P4_N5B5a_instance_mainline.md（v3）+ 本节，从 R1 开始」**
...
9. **（2026-09-08 N5b4-5a 执行会话：R1/R2 完成 + use_item 链设计文档）**
   ...
   - 🔴 **use_item 道具链设计（新会话执行蓝图）**：
     docs/archive/REFACTOR_v181P4_N5B5a_use_item_design.md——economy use_item
     ...
     **新会话口令：「读 docs/archive/REFACTOR_v181P4_N5B5a_use_item_design.md，
...
10. **（2026-09-08 会话尾）🔴 效果容器统一（V 系列）拍板——插队排期**：
  ...
  - **方案文档（完整字段级+排期）**：`docs/archive/REFACTOR_v181P4_EFFECTS_UNIFY_design.md`
...
- 方案：`docs/archive/REFACTOR_v181P4_EFFECTS_UNIFY_design.md`（完整字段级 + V1-V7 排期）
...
- 开工口令：「继续 effects 统一，读 docs/archive/REFACTOR_v181P4_EFFECTS_UNIFY_design.md
  从 V1 开始」
...
- **详细设计文档**：`docs/archive/REFACTOR_v181P4_N5B4_command_switch.md`（字段/函数级施工图）
...
- **N5b4-5 进度**：侦查发现副本深度定制旧引擎（仇恨/团队广播/剧本/内聚引用），
  切 battle2 需先补缺口 → 设计文档 `docs/archive/REFACTOR_v181P4_N5B5_instance_gap_design.md`
...
### D. N10 删旧（最终验收"清干净"）
- 删除清单见 `docs/archive/REFACTOR_v181P4_N9_migration.md` §4：battle.py（11000+ 行）/
...
- 🔴 **L3 玩家事件层已排期**（2026-09-09 拍板，N10 删旧合并 master 后开工）：
  设计+排期 = docs/archive/DESIGN_v181_L3_player_event_bus.md。L1 战斗内效果总线(N8) ✅ 已落地、
...
- 🔴 **L3 玩家事件层 P0-P4 全部完成（2026-09-09，全批同日收官）**：
  - P0 字段级任务书 docs/archive/REFACTOR_v181_L3_P0_task.md（339e90f）——侦察修订：手动接线在
