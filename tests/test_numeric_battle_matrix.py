...
# 残留待办（非 CTB）：① 同级 dps 怪偏脆→物理系 8/8 满胜、法/牧纯智普攻 0/8（法牧普攻体系弱，独立议题）
# ② +5 级(16) 物理系仍 8/8 碾压（目标 ≤6，平衡标定时收紧）
EXPECT = {
    # 同 11v11：目标 3..7/8，实测物理系满胜/法牧全败 → 锁定现状（平衡标定待办）
    ("战士", 11):   ("exact", 8, None, "⚠️同级 dps 偏脆（标定待办，目标 ≤7）"),
    ("法师", 11):   ("exact", 0, None, "⚠️法师普攻体系弱（独立议题，目标 ≥3）"),
    ("游侠", 11):   ("exact", 8, None, "⚠️同级 dps 偏脆（标定待办，目标 ≤7）"),
    ("牧师", 11):   ("exact", 0, None, "⚠️牧师普攻体系弱（独立议题，目标 ≥3）"),
    ("刺客", 11):   ("exact", 8, None, "⚠️同级 dps 偏脆（标定待办，目标 ≤7）"),
    ("拳师", 11):   ("exact", 8, None, "⚠️同级 dps 偏脆（标定待办，目标 ≤7）"),
    # +5 11v16：目标 ≤6/8，物理系仍碾压
    ("战士", 16):   ("exact", 8, None, "⚠️高 5 级仍碾压（标定待办，目标 ≤6）"),
    ("法师", 16):   ("exact", 0, None, ""),                  # 普攻弱同 11v11
    ("游侠", 16):   ("exact", 8, None, "⚠️高 5 级仍碾压（标定待办，目标 ≤6）"),
    ("牧师", 16):   ("exact", 0, None, ""),                  # 普攻弱同 11v11
    ("刺客", 16):   ("exact", 7, None, "⚠️高 5 级仍碾压（标定待办，目标 ≤6）"),
    ("拳师", 16):   ("exact", 8, None, "⚠️高 5 级仍碾压（标定待办，目标 ≤6）"),
    # -5 11v6：目标 ≥5/8
    ("战士", 6):    ("band", 5, 8, ""),                      # ✅
    ("法师", 6):    ("exact", 2, None, "⚠️法师普攻体系弱（独立议题，目标 ≥5）"),
    ("游侠", 6):    ("band", 5, 8, ""),                      # ✅
    ("牧师", 6):    ("band", 5, 8, ""),                      # ✅
    ("刺客", 6):    ("band", 5, 8, ""),                      # ✅
    ("拳师", 6):    ("band", 5, 8, ""),                      # ✅
    ...
    ("游侠", 22):   ("band", 0, 2, ""),
    ("牧师", 22):   ("band", 0, 2, ""),
    ("刺客", 22):   ("band", 0, 2, "✅ v130.10 CTB 修复后从 7/8 降至 0/8（跨 11 级必死）"),
    ("拳师", 22):   ("band", 0, 2, ""),
}
