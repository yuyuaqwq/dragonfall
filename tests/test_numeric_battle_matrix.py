...
# v169.3 承伤修复（2026-09-03，鱼鱼拍板）：怪 atk growth 上调（dps 5.0→9.0）+ atk_stage_mult 取消
# 负斜率 → 裸装 11v11 不再 8/8（怪单发对裸装玩家翻倍：战士 2/8、拳师 3/8 平均 10~12 轮才分胜负——
# 裸装同级从"稳过"变"势均力敌/小劣"，符合怪打玩家方向增强的设计意图；满装（蓝+0/蓝+5）11v11 仍 8/8
# 且 4.9~6.9 轮（满装玩家不受影响）。战士/拳师 11v11 从 exact 8 改为区间 band [0,8] 锁定现状。
# 残留待办（非 v131/v169.3）：法/牧纯智普攻贫血（11v6 也 0/8）——技能轴才是法系输出管道，独立议题二期。
EXPECT = {
    # 同 11v11：目标 3..7/8；v131 后物理 8/8 但轮数 6.1（不再秒杀）→ 锁定
    # v169.3 怪攻上调后裸装战士/拳师 2~3/8（势均力敌，非碾压）→ 区间 [0,8] 防过度红
    ("战士", 11):   ("band", 0, 8, "v169.3 怪攻上调：裸装 11v11 实测 2/8（满装仍 8/8），同级从碾压变势均力敌"),
    ("法师", 11):   ("band", 0, 4, "v174 普攻技能化：法系普攻改 matk（#75），11v11 从 0/8 提升到 1/8（不再刮痧但非碾压）"),
    ("游侠", 11):   ("band", 0, 8, "✅ v161 幂曲线：全敏游侠普攻不再碾压（spd 收益缓、atk 低），靠技能/连击输出；纯普攻同级 0~8/8 随 seed 波动"),
    ("牧师", 11):   ("band", 0, 4, "v174 普攻技能化：牧师普攻改 matk（#75），11v11 提升（不再 0/8 刮痧）"),
    ("刺客", 11):   ("band", 0, 8, "✅ v161 幂曲线：全敏刺客普攻同级 0~8/8（spd 收益缓、atk 低），连段/毒输出在技能轴；纯普攻不再 8/8"),
    ("拳师", 11):   ("band", 0, 8, "v169.3 怪攻上调：裸装 11v11 实测 3/8（满装仍 8/8）"),
    ...
    # -5 11v6：目标 ≥5/8
    ("战士", 6):    ("band", 5, 8, ""),                      # ✅
    ("法师", 6):    ("band", 5, 8, "v174 普攻技能化：法系普攻改 matk（#75），11v6 从 0/8 提升到 8/8（低级怪可碾压，符合预期）"),
    ("游侠", 6):    ("band", 5, 8, ""),                      # ✅
    ("牧师", 6):    ("band", 5, 8, "v174 普攻技能化：牧师普攻改 matk（#75），11v6 提升（低级怪可胜）"),
    ("刺客", 6):    ("band", 5, 8, ""),                      # ✅
    ("拳师", 6):    ("band", 5, 8, ""),                      # ✅
