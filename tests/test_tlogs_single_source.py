...
def scan_literal(root_rel: str, needle: str = "tlogs.json") -> tuple:
    ...
    """
    root = os.path.join(PLUGIN_DIR, root_rel)
    code_hits, doc_hits = [], []
...
def t4_single_read_point():
    ...
    pkg_code, pkg_doc = scan_literal(PKG_RUNTIME_REL)
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--plugin", default=PLUGIN_DIR,
                    help="宿主插件根（读点扫描/引擎装载按它走；默认 = 本文件上一级）")
    ap.add_argument("--pkg", default=None, help="真源（包内）；默认 <plugin>/%s" % PKG_REL)
    ap.add_argument("--host", default=None, help="镜像（宿主）；默认 <plugin>/%s" % HOST_REL)
    args = ap.parse_args(argv)

    PLUGIN_DIR = os.path.abspath(args.plugin)
    pkg_path = args.pkg or os.path.join(PLUGIN_DIR, PKG_REL)
