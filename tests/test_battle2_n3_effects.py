...
def test_debuff_stack():
    print("【N3.1 debuff 叠层：burn/bleed/poison cap 与写入】")
    b = BT_NEW(btype="monster", sides={"player": [], "enemy": []})
    caster = {"uid": "p", "name": "勇者", "buffs": {}, "stacks": {}}
    target = {"uid": "e", "name": "怪", "buffs": {}, "debuffs": {}, "hp": 100, "max_hp": 100}
    logs = []
    # burn 叠 2 层
    FX.apply_effects(b, caster, target, [{"type": "burn", "stacks": 2}], logs)
    burn = target["debuffs"].get("burn")
    check("burn 写入 debuffs", isinstance(burn, dict) and burn["n"] == 2, f"burn={burn}")
    # burn 再叠 4 层 → cap 5
    FX.apply_effects(b, caster, target, [{"type": "burn", "stacks": 4}], logs)
    check("burn cap 5", target["debuffs"]["burn"]["n"] == 5,
          f"n={target['debuffs']['burn']['n']}")
    # bleed 叠 3
    FX.apply_effects(b, caster, target, [{"type": "bleed", "stacks": 3}], logs)
    check("bleed 写入 debuffs", target["debuffs"].get("bleed", {}).get("n") == 3)
    # poison 免疫
    target["immune_dots"] = ["poison"]
    FX.apply_effects(b, caster, target, [{"type": "poison", "stacks": 2}], logs)
    check("poison 免疫不叠", "poison" not in target["debuffs"])
    del target["immune_dots"]
...
def test_control():
    ...
    target = {"uid": "e", "name": "怪", "buffs": {}, "debuffs": {}}
    logs = []
    ...
    # Boss 控制减半
    boss = {"uid": "boss", "name": "Boss", "is_boss": True, "buffs": {}, "debuffs": {}}
...
def test_caster_stack():
    print("【N3.3 叠层：zhan_yi/rage/chi 写入 caster】")
    b = BT_NEW(btype="monster", sides={"player": [], "enemy": []})
    caster = {"uid": "p", "name": "勇者", "buffs": {}, "stacks": {}, "resources": {}}
    logs = []
    FX.apply_effects(b, caster, None, [{"type": "zhan_yi", "stacks": 3}], logs)
    check("zhan_yi 3 层", caster["stacks"].get("zhan_yi") == 3)
    FX.apply_effects(b, caster, None, [{"type": "zhan_yi", "stacks": 9}], logs)
    check("zhan_yi cap 10", caster["stacks"].get("zhan_yi") == 10,
          f"n={caster['stacks'].get('zhan_yi')}")
    FX.apply_effects(b, caster, None, [{"type": "rage", "stacks": 4}], logs)
    check("rage 4", caster["resources"].get("rage") == 4)
    FX.apply_effects(b, caster, None, [{"type": "chi", "stacks": 5}], logs)
    check("chi 5", caster["resources"].get("chi") == 5)
...
def test_buff_effect_handler():
    ...
    b = BT_NEW(btype="monster", sides={"player": [], "enemy": []})
    caster = {"uid": "p", "name": "勇者", "buffs": {}, "stacks": {}, "resources": {},
              "shields": {}, "debuffs": {}, "reduce_left": 0, "max_hp": 1000, "hp": 500}
    logs = []
...
def test_mech_on_hit():
    ...
    if sk2 and info2.get("mech") == "burn":
        ...
        burn = m2.get("debuffs", {}).get("burn")
        check("龙息之怒命中后目标 burn ≥1",
              isinstance(burn, dict) and burn.get("n", 0) >= 1, f"burn={burn}")
    else:
