...
覆盖：
  1. make_gear upgrade 生效：upgrade=10 面板 > upgrade=0（升级倍率 1.35 叠加）
...
def main():
    print("【1/4 升级生效：upgrade=10 面板 > upgrade=0】")
    ...
    check("upgrade=10 面板 atk > upgrade=0（升级倍率 1.35 生效）", a10 > a0,
          f"up={a0} up10={a10}")
    # 纯装备差口径（去掉基础 atk）：upgrade 倍率 1.35 直接放大装备面板
    g_only0, g_only10 = a0 - base, a10 - base
    check("upgrade=10 装备差 ≥ upgrade=0 ×1.30（1.35 打折至 1.30 容差）",
          g_only10 >= g_only0 * 1.30, f"ratio={g_only10 / max(g_only0, 1):.3f}")
    w10 = g10["weapon"]
