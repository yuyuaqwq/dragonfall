...
check("战士怒气初始 0", b._p_res().get('rage') == 0, str(b._p_res()))
pm = mk('法师')
bm = BT.Battle('monster', mkmon('史莱姆'), player=pm)
check("法师元素亲和默认火", bm._p_res().get('element') == 'fire', str(bm._p_res()))
py = mk('游侠')
...
check("怒气上限 10", b4._p_res().get('rage') == 10, str(b4._p_res()))
...
check("精力 82 回 18 → 满 100（不排气，v153 专注流量制）", b2._p_res().get('energy') == 100, str(b2._p_res()))
py2 = mk('游侠')
by2 = BT.Battle('monster', mkmon(), player=py2)
by2._p_res()['energy'] = 50
logs = by2._tick_regen(py2, [])
check("精力每回合 +18", by2._p_res().get('energy') == 68, str(by2._p_res()))
check("回复日志", any("精力回复" in x for x in logs), str(logs))

print("【核心资源：消耗】")
assert E.core_resource_spend('游侠', by2._p_res(), 20) is True
check("消耗 20 精力", by2._p_res().get('energy') == 48, str(by2._p_res()))
assert E.core_resource_spend('游侠', by2._p_res(), 999) is False
check("不足不扣", by2._p_res().get('energy') == 48, str(by2._p_res()))
