...
def test_threshold():
    ...
    _first_hostile(b2).setdefault("debuffs", {})["poison"] = {"n": 9, "mult": 1.0}
    for _ in range(5):
        tick(b2, p)
    d2 = _first_hostile(b2)["debuffs"]["poison"]
...
def test_max_trigger_and_saturation():
    ...
    b2 = BT.Battle("monster", mk_enemy(hp=100000))
    _first_hostile(b2).setdefault("buffs", {})["stun"] = 1
    _first_hostile(b2)["buffs"]["stun_trigger_count"] = 1  # 已触发 1 次（<上限 2）
    tick(b2, p)
    check("stun 未饱和时 eb 保留（第 1 次）", _first_hostile(b2)["buffs"].get("stun") == 1,
          str(_first_hostile(b2)["buffs"]))
    _first_hostile(b2)["buffs"]["stun_trigger_count"] = 2  # 达上限 2
    tick(b2, p)
    check("stun 饱和后 eb 控制被移除（不再生效）", "stun" not in _first_hostile(b2)["buffs"],
          str(_first_hostile(b2)["buffs"]))
    # freeze/sleep 饱和
    for k in ("freeze", "sleep"):
        b3 = BT.Battle("monster", mk_enemy(hp=100000))
        _first_hostile(b3).setdefault("buffs", {})[k] = 1
        _first_hostile(b3)["buffs"][f"{k}_trigger_count"] = 2  # 达上限
        tick(b3, p)
        check(f"{k} 饱和后 eb 控制被移除", k not in _first_hostile(b3)["buffs"],
              str(_first_hostile(b3)["buffs"]))
...
def test_preserve():
    ...
    b2 = BT.Battle("monster", mk_enemy(hp=100000))
    b2._focus = p
    _first_hostile(b2).setdefault("debuffs", {})["bleed"] = {"n": 4, "mult": 1.0, "threshold": 2.9}
    b2._preserve_debuffs([])
    check("阈值 +15% 封顶 3.0", abs(_first_hostile(b2)["debuffs"]["bleed"]["threshold"] - 3.0) < 1e-9,
          str(_first_hostile(b2)["debuffs"]["bleed"]))
...
def test_true_dmg():
    ...
    tick(b2, p)
    check("同配置毒被防御+抗性双重压制（对照）", 1000 - _first_hostile(b2)["hp"] < 56,
          f"dmg={1000 - _first_hostile(b2)['hp']}")
    # 无 dot_res 时腐蚀全额 56（真伤不吃 def）
    b5 = BT.Battle("monster", mk_enemy(hp=1000, **{"def": 1000000, "mdef": 1000000}))
    _first_hostile(b5).setdefault("debuffs", {})["corros"] = {"n": 1, "mult": 1.0}
    tick(b5, p)
    check("无抗性时腐蚀全额 56（真伤豁免 def）", 1000 - _first_hostile(b5)["hp"] == 56,
          f"dmg={1000 - _first_hostile(b5)['hp']}")
    # 免疫仍生效
    b3 = BT.Battle("monster", mk_enemy(hp=1000, immune_dots=["corros"]))
    _first_hostile(b3).setdefault("debuffs", {})["corros"] = {"n": 3, "mult": 1.0}
    tick(b3, p)
    check("腐蚀仍走免疫检查", "corros" not in _first_hostile(b3).get("debuffs", {}),
          str(_first_hostile(b3).get("debuffs")))
    ...
    _first_hostile(b4).setdefault("debuffs", {})["corros"] = {"n": 1, "mult": 1.0}
    tick(b4, p)
    check("真伤护盾层全额吸收(不halve)盾值消耗", 1000 - _first_hostile(b4)["hp"] == 0
          and _first_hostile(b4)["shields"]["legacy"]["value"] == 444,
          f"hp={_first_hostile(b4)['hp']} shield={_first_hostile(b4).get('shields')}")
...
def test_saturate_conv():
    ...
    hp0 = _first_hostile(b2)["hp"]
    tick(b2, p)
    check("未饱和同层对照：n=4 满伤 320", hp0 - _first_hostile(b2)["hp"] == 320,
          f"dmg={hp0 - _first_hostile(b2)['hp']}")
...
def test_legacy_compat():
    ...
    b2 = BT.Battle("monster", mk_enemy(hp=100000))
    _first_hostile(b2).setdefault("debuffs", {})["burn"] = {"n": 3, "mult": 1.0}
    tick(b2, p)
    check("burn 旧结构兼容", _first_hostile(b2)["debuffs"]["burn"].get("trigger_count") == 1,
          str(_first_hostile(b2)["debuffs"]["burn"]))
