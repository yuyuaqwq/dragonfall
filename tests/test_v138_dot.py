...
def test_true_dmg():
    ...
    # 腐蚀 1 层：atk×0.4 + matk×0.3 + max_hp×2% = 40+24+20 = 84
    # 真伤只豁免防御削减（def/mdef），不豁免目标异常抗性（dot_res 0.9 仍生效 → 84×0.1=8）
    ...
    tick(b, p)
    check("腐蚀真伤绕过 def/mdef：84×0.1=8", 1000 - b.enemy["hp"] == 8,
...
def test_saturate_conv():
    ...
    d = b.enemy["debuffs"]["poison"]
    check("饱和置位当次满伤（n=4×1550=6200）", hp0 - b.enemy["hp"] == 6200,
          f"dmg={hp0 - b.enemy['hp']} n={d.get('n')}")
    check("置位后 saturate_mult=0.8（下次起收敛）", abs(d.get("saturate_mult", 0) - 0.8) < 1e-9,
          str(d))
    # 第 6 次：n=3，×0.8 → 1550×3×0.8 = 3720
    hp0 = b.enemy["hp"]
    tick(b, p)
    d = b.enemy["debuffs"]["poison"]
    check("饱和后同层伤害 ×0.8：1550×3×0.8=3720", hp0 - b.enemy["hp"] == 3720,
          f"dmg={hp0 - b.enemy['hp']} n={d.get('n')}")
    check("saturate_mult 继续收敛 0.64", abs(d.get("saturate_mult", 0) - 0.64) < 1e-9, str(d))
    # 第 7 次：n=2，×0.64 → 1550×2×0.64 = 1984
    hp0 = b.enemy["hp"]
    tick(b, p)
    check("二次收敛 ×0.64：1550×2×0.64=1984", hp0 - b.enemy["hp"] == 1984,
          f"dmg={hp0 - b.enemy['hp']}")
    # 对照：未饱和同层数（n=4）满伤 6200（无收敛乘区）
    b2 = BT.Battle("monster", mk_enemy(hp=100000))
    b2.enemy.setdefault("debuffs", {})["poison"] = {"n": 4, "mult": 1.0}
    hp0 = b2.enemy["hp"]
    tick(b2, p)
    check("未饱和同层对照：n=4 满伤 6200", hp0 - b2.enemy["hp"] == 6200,
