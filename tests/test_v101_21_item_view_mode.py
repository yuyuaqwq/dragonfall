# -*- coding: utf-8 -*-
"""v101.21 物品查看模式测试：『查看物品开始』→ 裸数字查看物品 → 『查看物品结束』退出。

覆盖：
1. 『查看物品开始』开启物品查看模式（event_state 落库）
2. 开启后裸数字 → 查看物品详情（背包序号）
3. 『查看物品结束』关闭 → 裸数字回退 NPC 对话
4. 『查看物品 <名称>』直接查看（主名），『物品详情 <名称>』旧名兼容
5. 开关 regex 只匹配开始/结束，不抢『查看物品 <名称>』
"""
...
async def main():
    ...
    # ---- 1. 『查看物品开始』开启 ----
    ev = FakeEvent("g1", "1001", "查看物品开始")
    r = "".join(str(x) for x in await run(m.item_view_mode_cmd, ev))
    print("  [查看物品开始]", r[:80].replace("\n", " | "))
    check("『查看物品开始』响应", "已开启" in r, r[:80])
    ...
    # ---- 3. 『查看物品结束』关闭 ----
    ev3 = FakeEvent("g1", "1001", "查看物品结束")
    r3 = "".join(str(x) for x in await run(m.item_view_mode_cmd, ev3))
    check("『查看物品结束』响应", "已关闭" in r3, r3[:80])
    ...
    # ---- 5. 『查看物品 <名称>』直接查看（主名）+ 旧名兼容 ----
    ev5 = FakeEvent("g1", "1001", "查看物品 草药茶")
    r5 = "".join(str(x) for x in await run(m.item_detail, ev5))
    print("  [查看物品 草药茶]", r5[:100].replace("\n", " | "))
    check("『查看物品 <名称>』查看", "草药茶" in r5, r5[:100])
    ev6 = FakeEvent("g1", "1001", "物品详情 草药茶")
    r6 = "".join(str(x) for x in await run(m.item_detail, ev6))
    check("旧名『物品详情』兼容", "草药茶" in r6, r6[:100])

    # ---- 6. regex 互斥 ----
    sw_re = re.compile(r"^(?:\[At:\d+\]\s*)?查看物品(?:开始|结束)(?:\s*|$)")
    dt_re = re.compile(r"^(?:\[At:\d+\]\s*)?(?:查看物品|物品详情)(?:[\s\S]*)$")
    check("『查看物品开始』匹配开关", bool(sw_re.match("查看物品开始")), "开关 regex 未匹配")
    check("『查看物品 铁剑』不匹配开关", not sw_re.match("查看物品 铁剑"), "开关 regex 误吞查看指令")
    check("『查看物品 铁剑』匹配详情", bool(dt_re.match("查看物品 铁剑")), "详情 regex 未匹配")
    check("『物品详情 1』匹配详情", bool(dt_re.match("物品详情 1")), "旧名 regex 未匹配")
