...
async def main():
    ...
    out = await cmd(m, "mining", "g1", "w1", "采矿")
    check("采矿自动激活2/2", "选择了「采矿」" in out and "2/2" in out, out[:200])
    m._prof_wait_clear("g1", "w1")
    ...
    out = await cmd(m, "fishing", "g1", "w1", "钓鱼")
    check("遗忘后可再激活", "选择了「钓鱼」" in out, out[:200])
    m._prof_wait_clear("g1", "w1")
    ...
    await cmd(m, "prof_forget", "g1", "w1", "遗忘副业 采矿")
    out = await cmd(m, "cooking", "g1", "w1", "烹饪 鱼汤")
