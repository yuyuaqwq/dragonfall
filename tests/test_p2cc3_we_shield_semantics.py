...
def test_routing():
    ...
    # 未迁移 key（proc_buff/proc_heal regen 等未迁移）→ 不在路由 → 仍走旧 handler
    # （C7 已迁 trinity_rhythm/guardian_will 等 9 key → 从"未族化"清单移除，换 proc_heal regen
    #  guard_regen 与 proc_buff gale_step 代表；C4/C8 已迁 wind_split/twilight_execute 等）
    for k in ("guard_regen", "gale_step"):
