...
# ⚠️ 导入顺序铁律：第一个必须 import game.content（data→core→content 标准完整路径，
...
def _mk_gem(stats, tier):
    """构造一颗指定 stats/tier 的原石 dict（测试夹具）。"""
    return {"name": "测试原石", "type": "原石", "gem": True, "stats": stats, "tier": tier, "icon": "💎"}
...
def test_data_tables():
    ...
    assert GEM_TIER_NAMES[1] == "碎裂I" and GEM_TIER_NAMES[10] == "传说II"
...
def test_roll_gem_structure():
    ...
    for _ in range(200):
        ...
        assert g.get("type") == "原石" and g.get("gem") is True
