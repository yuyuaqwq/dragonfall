...
def test_data():
    ...
    check("名册 682 件", len(C.EQUIP_ROSTER) == 682, str(len(C.EQUIP_ROSTER)))  # v124: +6 支线; v136 Phase6: +180; v140: +213; v141.3: +龙鳞庇护之坠; v168: +8 新手本 Boss 主题装; v169: +12 断档补装; v171: +6 新手流派白装; v172 路B: +24 重锻专属(壁垒/铭文/贤者/巡猎/夜行/镇岳系)
...
def test_battle_affix():
    ...
    check("护盾词条战斗开始", b.p_shields.get("affix_shield", {}).get("value") == int(int(p["max_hp"] * 0.10) * 1.05)
          and abs(float(b.p_shields.get("affix_shield", {}).get("expire_at", 0)) - 3.0) < 1e-9, str(b.p_shields))  # v106.2 战士盾强 5%；v152 ACT_TICK=1.0：3 刻 = 3.0
    # 6.2 处决低血增伤（血 20% 触发）
