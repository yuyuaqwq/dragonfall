...
async def main():
    ...
    out = await cmd(m, "evolve", "g1", "p4", "转职 奥法大师")
    check("40 级转奥术大师(T2)被拒", "Lv.60" in out, out[:150])
    out = await cmd(m, "evolve", "g1", "p4", "转职 大魔导师")
    ...
    out = await cmd(m, "evolve", "g1", "p5", "转职 大魔导师")
