...
async def main():
    ...
    for s in range(600):
        random.seed(s)
        for mk, mn in orange_rec["mats"].items():
            if _count("g1", "q1", mk) < mn:
                db.add_item("g1", "q1", mk, {"name": C.display("materials", mk), "type": "材料",
                                             "stackable": True, "price": 1}, mn)
        ...
        if "精良作品" in out:
            ...
            break
    # 橙装 2% 精良命中：600 seed 样本；4% 概率（2%×~95%）在 600 内不中概率 e^-24≈0，必现
    check("橙装 2% 精良命中（600 seed 内出现）", hit3, "未命中")
    ...
    # 统计多 seed 命中率（7% vs 5% 用大样本区分度有限，验证阈值边缘：roll 值 ∈ [0.05, 0.07) 应触发）
    hit10 = 0
    total10 = 0
    for s in range(120):
        random.seed(s)
        for mk, mn in blue_rec["mats"].items():
            if _count("g1", "q1", mk) < mn:
                db.add_item("g1", "q1", mk, {"name": C.display("materials", mk), "type": "材料",
                                             "stackable": True, "price": 1}, mn)
        # 每轮补齐品质材料
        if _count("g1", "q1", "mat_jing_jin") < 1:
            db.add_item("g1", "q1", "mat_jing_jin", {"name": "精金", "type": "材料", "stackable": True, "price": 1}, 3)
        if _count("g1", "q1", "mat_shen_hai_shui_jing") < 1:
            db.add_item("g1", "q1", "mat_shen_hai_shui_jing", {"name": "深海水晶", "type": "材料", "stackable": True, "price": 1}, 3)
        out = await cmd(m5, "craft", "g1", "q1", f"锻造 {C.display('recipes', blue_key)}")
        total10 += 1
        if "品质升华" in out:
            hit10 += 1
    print(f"    Lv.10 神锻名家 120 次命中 {hit10}/{total10}")
    # 7% 命中率：120 次期望 8.4 次；阈值边缘（5%~7% 区间）随机波动容忍 3 次+
    # 分桶精确验证：Lv.10 时应比 Lv.6 命中的 seed 更多（等价阈值 0.07 vs 0.05）
    check("神锻名家命中率 > 5%（约 7%）", hit10 / max(total10, 1) >= 0.05, f"{hit10}/{total10}")
    # 分桶对照：同一批 seed 下 Lv.10 应覆盖 Lv.6 的命中 seed（7% 阈值 ⊇ 5% 阈值）
    print("    （阈值边缘 5%~7% 的 seed 才是 Lv.10 独有触发——无法靠随机样本稳定复现，上面按比率校验）")
