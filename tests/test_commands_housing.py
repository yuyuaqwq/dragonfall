...
async def main():
    ...
    check("买房成功", "恭喜置业" in out and "喷泉小屋" in out, out[:200])
    check("金币扣减", db.get_player("g1", "w1")["gold"] == 3000, str(db.get_player("g1", "w1")["gold"]))
    check("deed 已记录", db.get_player("g1", "w1").get("deed") == "prop_vila_01", str(db.get_player("g1", "w1").get("deed")))
    out = await cmd(m, "deed_buy", "g1", "w1", "买房 2")
    check("已有房拦截", "已经有一张地契" in out, out[:120])
    out = await cmd(m, "deed_view", "g1", "w1", "地契")
    check("地契大厅显示我的房产", "我的地契" in out and "回家" in out, out[:200])
    # 金币不足
    out = await cmd(m, "deed_buy", "g1", "w2", "买房 1")
    check("金币不足拦截", "需要 2000 金币" in out, out[:120])
    ...
    check("家地图面板", "我的家" in out and "铺面空着" in out and "仓库" in out, out[:300])
    ...
    check("卖房退一半", "退还 1000 金币" in out, out[:120])
