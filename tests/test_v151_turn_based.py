...
def _sim_hit(battle, logs):
    ...
    hits = getattr(battle, "_p_buff_hits", None)
    if not hits:
        return
    for _hk in [k for k in list(hits) if int(hits.get(k, 0) or 0) > 0]:
        ...
        if _nh <= 0:
            hits.pop(_hk, None)
            if _hk in battle.p_buffs:
                battle.p_buffs.pop(_hk, None)
...
def _sim_end_round(battle):
    ...
    for tbl in (battle.p_buffs,):
        for k in list(tbl):
            if k in (getattr(battle, "_p_buff_hits", {}) or {}):
                continue  # v151 时刻制豁免
            tbl[k] -= 1
            if tbl[k] <= 0:
                del tbl[k]
...
def main():
    ...
    _sim_end_round(b)
    check("回合结束豁免：def_up 不递减", b.p_buffs.get("def_up") == 3,
          str(b.p_buffs))
    ...
    check("受击 1 次：def_up 仍在（hits 2/3）",
          b.p_buffs.get("def_up") == 3 and b._p_buff_hits["def_up"] == 2,
          str((b.p_buffs, b._p_buff_hits)))

    _sim_hit(b, logs)
    _sim_hit(b, logs)  # 受击 3 次
    check("受击 3 次：def_up 消失",
          "def_up" not in b.p_buffs and "def_up" not in b._p_buff_hits,
          str((b.p_buffs, b._p_buff_hits)))
    ...
    b2._p_buff_hits = {}
    _sim_end_round(b2)
    check("攻击 buff 不豁免：回合递减", b2.p_buffs.get("atk_up") == 2,
          str(b2.p_buffs))
